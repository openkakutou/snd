...
## Acceptance Criteria
- [ ] `GOOS=js GOARCH=wasm` build produces a working `snd.wasm` + `wasm_exec.js`, exposing a version-agnostic decode call (v1/v2 auto-detected, mirroring `sff.Load`'s own auto-detection) as a JS-callable global
- [ ] A Node-based smoke test verifies it loads and decodes a real fixture sound file
- [ ] A GitHub Actions workflow publishes both artifacts on every tag, mirroring `sff`'s release workflow
